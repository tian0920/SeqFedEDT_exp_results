...
base_folder = "results/alpha=0.1/score"  # Change to the actual folder path
...
for dataset_idx, dataset_name in enumerate(dataset_names):
    ...
    for idx, (method_name, (x_vals, y_vals)) in enumerate(dataset_methods.items()):
        ...
        filtered_x_vals = [x for x in x_vals if 0.999 <= x <= 1]
        filtered_y_vals = [y_vals[i] for i in range(len(x_vals)) if 0.999 <= x_vals[i] <= 1]
...
# 保存图像到文件夹
output_path = os.path.join(output_dir, 'score_mnist.pdf')
